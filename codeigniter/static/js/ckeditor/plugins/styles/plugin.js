...
(function() {
    ...
    CKEDITOR.style.prototype = {apply:function(A) {
        ...
        switch (this.type) {case CKEDITOR.STYLE_BLOCK:return this.checkElementRemovable(A.block || A.blockLimit, true);case CKEDITOR.STYLE_INLINE:var B = A.elements;for (var C = 0,D; C < B.length; C++) {
            D = B[C];
    ...
    function y(A, B) {
        ...
        return C.replace(/\s*([;:])\s*/, '$1').replace(/([^\s;])$/, '$1;').toLowerCase();
    }
